(function registerCalendar(global: Window & typeof globalThis) {
  ...
  function renderCalendar(containerElement: HTMLElement | null = null, targetDate: Date | null = null): void {
    ...
    const dailyViolations: Record<number, number> = {};
    employees.forEach(employee => employee.violations.forEach(v => {
      const normalized = normalizeDateString(v.date);
      const dateParts = normalized.split('-');
      const violationYear = parseInt(dateParts[0], 10);
      const violationMonth = parseInt(dateParts[1], 10) - 1;
      const violationDay = parseInt(dateParts[2], 10);
  ...
  function renderGreenCardCalendar(containerElement: HTMLElement | null = null, targetDate: Date | null = null): void {
    ...
    const dailyGreenCards: Record<number, number> = {};
    employees.forEach(employee => {
      if (employee.greenCards) {
        employee.greenCards.forEach(gc => {
          const normalized = normalizeDateString(gc.date);
          const dateParts = normalized.split('-');
          const cardYear = parseInt(dateParts[0], 10);
          const cardMonth = parseInt(dateParts[1], 10) - 1;
          const cardDay = parseInt(dateParts[2], 10);
