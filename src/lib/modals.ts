(function registerModals(global: Window & typeof globalThis) {
  ...
  function showDayDetailsModal(year: number, month: number, day: number): void {
    ...
    const violationsOnDay = employees.flatMap(e => {
      if (!e || !Array.isArray(e.violations)) return [];
      return e.violations
        .filter(v => v && v.date)
  ...
  function showDayDetailsModalGreen(year: number, month: number, day: number): void {
    ...
    const greenCardsOnDay = employees.flatMap(e => {
      if (!e || !Array.isArray(e.greenCards)) return [];
      return e.greenCards
    ...
    const dayTitle = translateArgs(
      I18N?.modals?.greenDayTitle || 'modals.greenDayTitle',
      { date: formatDate(clickedDate) },
      `Green cards on ${formatDate(clickedDate)}`
    );
  ...
  function openGiveCardModal(name: string | null = null): void {
    renderModals();
    if (name) {
      const dropdown = document.querySelector('#cardEmployeeDropdown');
      const toggle = dropdown?.querySelector('.custom-dropdown-toggle') as HTMLElement | null;
      const item = dropdown?.querySelector(`li[data-value="${name}"]`) as HTMLElement | null;
      if (toggle && item) {
  ...
  function openGiveGreenCardModal(name: string | null = null): void {
