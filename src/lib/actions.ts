(function registerActions(global: Window & typeof globalThis) {
  type RenderTargets = Partial<Record<string, boolean>>;

  function requestRender(targets: RenderTargets): void {
    if (typeof global.queueDashboardRender === 'function') {
      global.queueDashboardRender(targets);
    } else if (typeof global.renderAll === 'function') {
      global.renderAll();
    }
  }

  function flagEmployeesDirty(): void {
    if (typeof global.markEmployeesDirty === 'function') {
      global.markEmployeesDirty();
    }
  }
  function showSuccessMessage(buttonElement: HTMLButtonElement | null, message = 'Success'): void {
    if (!buttonElement) return;
    const originalHtml = buttonElement.innerHTML;
    buttonElement.disabled = true;
    buttonElement.innerHTML = `<span class="text-green-400">✓ ${message}</span>`;
    setTimeout(() => {
      buttonElement.innerHTML = originalHtml;
      buttonElement.disabled = false;
      lucide.createIcons();
    }, 2000);
  }

  async function addEmployee(btn: HTMLButtonElement): Promise<void> {
    const name = (document.getElementById('newEmpName') as HTMLInputElement).value.trim();
    const role = (document.getElementById('newEmpRole') as HTMLInputElement).value.trim();
    const dept = (document.getElementById('newEmpDept') as HTMLInputElement).value.trim();
    const email = (document.getElementById('newEmpEmail') as HTMLInputElement).value.trim();
    const discordId = (document.getElementById('newEmpDiscord') as HTMLInputElement).value.trim();
    const joinDate = new Date().toISOString().split('T')[0];
    if (!name || !role || !dept) { alert('Please fill in all required fields.'); return; }
    if (employees.some(e => e.name.toLowerCase() === name.toLowerCase())) { alert('An employee with this name already exists.'); return; }

    updateSaveStatus('Saving...', 'saving');

    const newEmployee: Employee = {
      name,
      role,
      dept,
      email,
      discordId,
      joinDate,
      violations: []
    };

    try {
      const remoteEmployee = await createEmployeeRemote(newEmployee);
      if (!remoteEmployee) {
        throw new Error('Empty response from API');
      }
      employees.push(remoteEmployee);
      flagEmployeesDirty();
      requestRender({
        stats: true,
        yellowTable: true,
        greenTable: true,
        team: true,
        leaderboard: true,
        modals: true,
        greenModals: true
      });
      showSuccessMessage(btn, 'Employee Added');
      updateSaveStatus('✓ Saved', 'success');
      setTimeout(() => closeModal('addEmployeeModal'), 500);
      (document.getElementById('newEmpName') as HTMLInputElement).value = '';
      (document.getElementById('newEmpRole') as HTMLInputElement).value = '';
      (document.getElementById('newEmpDept') as HTMLInputElement).value = '';
      (document.getElementById('newEmpEmail') as HTMLInputElement).value = '';
      (document.getElementById('newEmpDiscord') as HTMLInputElement).value = '';
    } catch (error: any) {
      console.error('Failed to create employee:', error);
      updateSaveStatus('✗ Failed to save', 'error');
      alert(`Failed to create employee: ${error.message || error}`);
    }
  }

  async function removeEmployee(btn: HTMLButtonElement): Promise<void> {
    const dropdown = document.querySelector('#removeEmpDropdown .custom-dropdown-toggle') as HTMLElement | null;
    const name = dropdown?.dataset.value;
    if (!name) { alert('Please select an employee to remove.'); return; }
    if (!confirm(`Are you sure you want to remove ${name}? This action is permanent.`)) {
      return;
    }
    const emp = employees.find(e => e.name === name);
    if (!emp) {
      alert('Employee not found.');
      return;
    }
    if (!emp.id && emp.id !== 0) {
      alert('Employee ID is missing. Cannot remove employee.');
      return;
    }

    updateSaveStatus('Saving...', 'saving');
    const success = await removeEmployeeRemote(emp.id);
    if (success) {
      const index = employees.indexOf(emp);
      if (index >= 0) {
        employees.splice(index, 1);
        flagEmployeesDirty();
      }
      requestRender({
        stats: true,
        yellowTable: true,
        greenTable: true,
        team: true,
        leaderboard: true,
        modals: true,
        greenModals: true
      });
      showSuccessMessage(btn, 'Employee Removed');
      updateSaveStatus('✓ Saved', 'success');
      setTimeout(() => closeModal('removeEmployeeModal'), 500);
    } else {
      updateSaveStatus('✗ Failed to save', 'error');
    }
  }

  function submitYellowCard(btn: HTMLButtonElement): void {
    const employeeToggle = document.querySelector('#cardEmployeeDropdown .custom-dropdown-toggle') as HTMLElement | null;
    const typeToggle = document.querySelector('#cardViolationTypeDropdown .custom-dropdown-toggle') as HTMLElement | null;
    const name = employeeToggle?.dataset.value;
    const type = typeToggle?.dataset.value;
    const comment = (document.getElementById('cardComment') as HTMLTextAreaElement).value.trim();

    if (!name || !type) { alert('Please select an employee and a violation type.'); return; }
    if (!comment) { alert('Please fill in the details field.'); return; }

    giveCard(name, type, comment);
    showSuccessMessage(btn, 'Card Issued');
    setTimeout(() => closeModal('giveCardModal'), 500);
  }

  async function giveCard(name: string, type: string, comment: string): Promise<void> {
    const emp = employees.find(e => e.name === name);
    if (!emp) return;
    const today = new Date();
    const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
    const violation: Violation = { date: dateStr, type, comment };

    updateSaveStatus('Saving...', 'saving');

    try {
      if (!emp.id && emp.id !== 0) {
        console.error('Employee ID is missing:', emp);
        updateSaveStatus('✗ Employee ID missing', 'error');
        return;
      }
      const success = await addViolationViaAPI(emp.id, violation);
      if (success) {
        emp.violations.push(violation);
        requestRender({
          stats: true,
          calendar: true,
          yellowTable: true,
          modals: true,
          leaderboard: true
        });
        updateSaveStatus('✓ Saved', 'success');
      } else {
        updateSaveStatus('✗ Failed to save', 'error');
      }
    } catch (error) {
      console.error('Failed to save via API:', error);
      updateSaveStatus('✗ Failed to save', 'error');
    }
  }

  function submitGreenCard(btn: HTMLButtonElement): void {
    const employeeToggle = document.querySelector('#greenCardEmployeeDropdown .custom-dropdown-toggle') as HTMLElement | null;
    const typeToggle = document.querySelector('#greenCardTypeDropdown .custom-dropdown-toggle') as HTMLElement | null;
    const name = employeeToggle?.dataset.value;
    const type = typeToggle?.dataset.value;
    const comment = (document.getElementById('greenCardComment') as HTMLTextAreaElement).value.trim();

    if (!name || !type) { alert('Please select an employee and a card type.'); return; }
    if (!comment) { alert('Please fill in the details field.'); return; }

    giveGreenCard(name, type, comment);
    showSuccessMessage(btn, 'Green Card Issued');
    setTimeout(() => closeModal('giveGreenCardModal'), 500);
  }

  async function giveGreenCard(name: string, type: string, comment: string): Promise<void> {
    const emp = employees.find(e => e.name === name);
    if (!emp) return;
    const today = new Date();
    const dateStr = `${today.getFullYear()}-${String(today.getMonth() + 1).padStart(2, '0')}-${String(today.getDate()).padStart(2, '0')}`;
    const greenCard: GreenCard = { date: dateStr, type, comment };

    updateSaveStatus('Saving...', 'saving');

    try {
      const result = await addGreenCardViaAPI(emp.id ?? 0, greenCard);
      if (result && result.success) {
        if (!emp.greenCards) {
          emp.greenCards = [];
        }
        greenCard.id = result.id;
        emp.greenCards.push(greenCard);
        requestRender({
          stats: true,
          greenCalendar: true,
          greenTable: true,
          greenModals: true,
          leaderboard: true
        });
        updateSaveStatus('✓ Saved', 'success');
      } else {
        updateSaveStatus('✗ Failed to save', 'error');
        alert('Failed to save green card. Please try again.');
      }
    } catch (error: any) {
      console.error('Failed to save via API:', error);
      updateSaveStatus('✗ Failed to save', 'error');
      alert(`Failed to save green card: ${error.message || error}. The green_cards table may need to be created in the database.`);
    }
  }

  async function saveEmployeeChanges(btn: HTMLButtonElement): Promise<void> {
    const originalName = (document.getElementById('originalEmpName') as HTMLInputElement).value;
    const newName = (document.getElementById('editEmpName') as HTMLInputElement).value.trim();
    const newRole = (document.getElementById('editEmpRole') as HTMLInputElement).value.trim();
    const newDept = (document.getElementById('editEmpDept') as HTMLInputElement).value.trim();
    const newEmail = (document.getElementById('editEmpEmail') as HTMLInputElement).value.trim();
    const newDiscordId = (document.getElementById('editEmpDiscord') as HTMLInputElement).value.trim();

    if (!newName || !newRole || !newDept) { alert('Please fill in all required fields.'); return; }

    const emp = employees.find(e => e.name === originalName);
    if (!emp) { alert('Employee not found.'); return; }
    if (!emp.id && emp.id !== 0) {
      alert('Employee ID is missing. Cannot update employee.');
      return;
    }

    if (employees.some(e => e.id !== emp.id && e.name.toLowerCase() === newName.toLowerCase())) {
      alert('An employee with this name already exists.');
      return;
    }

    updateSaveStatus('Saving...', 'saving');

    const payload: Employee = {
      ...emp,
      name: newName,
      role: newRole,
      dept: newDept,
      email: newEmail,
      discordId: newDiscordId
    };

    try {
      const success = await updateEmployeeRemote(payload);
      if (success) {
        emp.name = newName;
        emp.role = newRole;
        emp.dept = newDept;
        emp.email = newEmail;
        emp.discordId = newDiscordId;
        flagEmployeesDirty();
        requestRender({
          stats: true,
          yellowTable: true,
          greenTable: true,
          team: true,
          leaderboard: true,
          modals: true,
          greenModals: true
        });
        showSuccessMessage(btn, 'Changes Saved');
        updateSaveStatus('✓ Saved', 'success');
        setTimeout(() => closeModal('editEmployeeModal'), 500);
      } else {
        updateSaveStatus('✗ Failed to save', 'error');
      }
    } catch (error: any) {
      console.error('Error updating employee:', error);
      updateSaveStatus('✗ Failed to save', 'error');
      alert(`Failed to update employee: ${error.message || error}`);
    }
  }

  function openEmailClient(email?: string): void {
    if (email) {
      window.location.href = `mailto:${email}`;
    }
  }

  function openDiscordChat(discordId: string | undefined, name: string): void {
    if (discordId) {
      window.open(`https://discord.com/users/${discordId}`, '_blank');
    }
  }

  async function confirmDeleteViolation(violationId: number, employeeName: string, meta: Record<string, any>): Promise<void> {
    if (!confirm('Remove this yellow card?')) return;
    let payload: Record<string, any> | undefined;
    const hasId = Number.isFinite(violationId);
    if (hasId) {
      payload = { violationId };
    } else if (meta && meta.date && meta.type) {
      payload = { name: employeeName, date: meta.date, type: meta.type, comment: meta.comment || '' };
    } else {
      updateSaveStatus('✗ Cannot delete: missing identifier', 'error');
      return;
    }
    const ok = await deleteViolation(payload);
    if (ok) {
      const emp = employees.find(e => e.name === employeeName);
      if (emp) {
        if (hasId) {
          emp.violations = emp.violations.filter(v => v && v.id !== violationId);
        } else {
          emp.violations = emp.violations.filter(v => v && !(v.date === meta.date && v.type === meta.type && (v.comment || '') === (meta.comment || '')));
        }
      } else {
        employees.forEach(e => {
          if (hasId) {
            e.violations = e.violations.filter(v => v && v.id !== violationId);
          } else {
            e.violations = e.violations.filter(v => v && !(v.date === meta.date && v.type === meta.type && (v.comment || '') === (meta.comment || '')));
          }
        });
      }
      requestRender({
        stats: true,
        calendar: true,
        yellowTable: true,
        modals: true,
        leaderboard: true
      });
      updateSaveStatus('✓ Card removed', 'success');
      if (meta && meta.context === 'employee') {
        const updatedEmp = employees.find(e => e.name === employeeName);
        if (updatedEmp) {
          showEmployeeModal({
            name: updatedEmp.name,
            role: updatedEmp.role,
            dept: updatedEmp.dept,
            email: updatedEmp.email,
            discordId: updatedEmp.discordId,
            cards: updatedEmp.violations.length
          });
        } else {
          closeModal('employeeModal');
        }
      } else if (meta && meta.context === 'day' && meta.date) {
        const parts = meta.date.split('-');
        if (parts.length === 3) {
          const year = parseInt(parts[0], 10);
          const month = parseInt(parts[1], 10) - 1;
          const day = parseInt(parts[2], 10);
          if (!Number.isNaN(year) && !Number.isNaN(month) && !Number.isNaN(day)) {
            showDayDetailsModal(year, month, day);
          }
        }
      }
    } else {
      updateSaveStatus('✗ Failed to remove card', 'error');
    }
  }

  async function confirmDeleteGreenCard(greenCardId: number, employeeName: string, meta: Record<string, any>): Promise<void> {
    if (!confirm('Remove this green card?')) return;
    let payload: Record<string, any> | undefined;
    const hasId = Number.isFinite(greenCardId);
    if (hasId) {
      payload = { greenCardId };
    } else if (meta && meta.date && meta.type) {
      payload = { name: employeeName, date: meta.date, type: meta.type, comment: meta.comment || '' };
    } else {
      updateSaveStatus('✗ Cannot delete: missing identifier', 'error');
      return;
    }
    const ok = await deleteGreenCard(payload);
    if (ok) {
      const emp = employees.find(e => e.name === employeeName);
      if (emp && emp.greenCards) {
        if (hasId) {
          emp.greenCards = emp.greenCards.filter(gc => gc.id !== greenCardId);
        } else {
          emp.greenCards = emp.greenCards.filter(gc => !(gc.date === meta.date && gc.type === meta.type && (gc.comment || '') === (meta.comment || '')));
        }
      }
      requestRender({
        stats: true,
        greenCalendar: true,
        greenTable: true,
        greenModals: true,
        leaderboard: true
      });
      updateSaveStatus('✓ Green card removed', 'success');
      if (meta && meta.context === 'employee') {
        const updatedEmp = employees.find(e => e.name === employeeName);
        if (updatedEmp) {
          showEmployeeModal({
            name: updatedEmp.name,
            role: updatedEmp.role,
            dept: updatedEmp.dept,
            email: updatedEmp.email,
            discordId: updatedEmp.discordId,
            cards: updatedEmp.violations.length
          });
        } else {
          closeModal('employeeModal');
        }
      } else if (meta && meta.context === 'day' && meta.date) {
        const parts = meta.date.split('-');
        if (parts.length === 3) {
          const year = parseInt(parts[0], 10);
          const month = parseInt(parts[1], 10) - 1;
          const day = parseInt(parts[2], 10);
          if (!Number.isNaN(year) && !Number.isNaN(month) && !Number.isNaN(day)) {
            showDayDetailsModalGreen(year, month, day);
          }
        }
      }
    } else {
      updateSaveStatus('✗ Failed to remove green card', 'error');
    }
  }

  function copyToClipboard(text: string, sourceElement?: HTMLElement): void {
    navigator.clipboard.writeText(text).then(() => {
      if (sourceElement) {
        sourceElement.classList.add('copied');
        setTimeout(() => sourceElement.classList.remove('copied'), 1000);
      }
    }).catch(err => console.error('Failed to copy text: ', err));
  }

  document.addEventListener('click', (event) => {
    const target = event.target as HTMLElement;
    const btn = target.closest('.js-delete-violation') as HTMLElement | null;
    if (btn) {
      const violationId = btn.dataset.violationId ? Number.parseInt(btn.dataset.violationId, 10) : NaN;
      const employeeName = btn.dataset.employeeName || '';
      const meta = {
        context: btn.dataset.context,
        date: btn.dataset.date,
        type: btn.dataset.type,
        comment: btn.dataset.comment
      };
      confirmDeleteViolation(violationId, employeeName, meta);
      return;
    }

    const greenCardBtn = target.closest('.js-delete-green-card') as HTMLElement | null;
    if (greenCardBtn) {
      const greenCardId = greenCardBtn.dataset.greenCardId ? Number.parseInt(greenCardBtn.dataset.greenCardId, 10) : NaN;
      const employeeName = greenCardBtn.dataset.employeeName || '';
      const meta = {
        context: greenCardBtn.dataset.context,
        date: greenCardBtn.dataset.date,
        type: greenCardBtn.dataset.type,
        comment: greenCardBtn.dataset.comment
      };
      confirmDeleteGreenCard(greenCardId, employeeName, meta);
    }
  });

  global.addEmployee = addEmployee;
  global.removeEmployee = removeEmployee;
  global.submitYellowCard = submitYellowCard;
  global.submitGreenCard = submitGreenCard;
  global.giveCard = giveCard;
  global.giveGreenCard = giveGreenCard;
  global.saveEmployeeChanges = saveEmployeeChanges;
  global.openEmailClient = openEmailClient;
  global.openDiscordChat = openDiscordChat;
  global.confirmDeleteViolation = confirmDeleteViolation;
  global.confirmDeleteGreenCard = confirmDeleteGreenCard;
  global.copyToClipboard = copyToClipboard;
})(window);


