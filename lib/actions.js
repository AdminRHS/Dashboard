(function (global) {
    ...
    async function confirmDeleteViolation(violationId, employeeName) {
        console.log('confirmDeleteViolation clicked', { violationId, employeeName });
        if (!violationId) return;
        if (!confirm('Remove this yellow card?')) return;
        const ok = await deleteViolation(violationId);
        if (ok) {
            // Remove locally
    ...
    document.addEventListener('click', (event) => {
        ...
        if (!btn) return;
        const violationId = parseInt(btn.dataset.violationId, 10);
        const employeeName = btn.dataset.employeeName || '';
        confirmDeleteViolation(violationId, employeeName);
    });
