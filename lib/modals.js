(function (global) {
    ...
    function showDayDetailsModal(year, month, day) {
            ...
            const violationsOnDay = employees.flatMap(e => {
                if (!e || !e.violations || !Array.isArray(e.violations)) return [];
                return e.violations
                    .filter(v => v && v.date)
                    .filter(v => {
                        const raw = v.date;
                    ...
                    .filter(v => v.type); // Filter out any undefined violations
            });
