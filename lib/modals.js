                ...
                violationsOnDay.forEach(v => {
                    ...
                    content += `
                        <div class="flex items-start gap-3">
                            ...
                            <button class="ml-2 text-red-600 hover:text-red-700 js-delete-violation" data-context="day" data-violation-id="${Number.isFinite(v.id) ? v.id : ''}" data-employee-name="${v.name.replace(/\"/g, '&quot;')}" data-date="${dateStr}" data-type="${v.type}" data-comment="${(v.comment || '').replace(/\"/g, '&quot;')}" title="Remove card"><i data-lucide=\"trash-2\" class=\"w-4 h-4\"></i></button>
                        </div>
                    `;
                });
                content += '</div>';
