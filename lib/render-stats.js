(function (global) {
    ...
    function renderStats() {
            ...
            const formatValue = (value) => value >= 3 ? '3+' : `${value}`;
            
            const greenCardActual = totalGreenCards;
            const yellowCardActual = totalCards;
            const orangeCardActual = cardCounts[2];
            const redCardActual = cardCounts[3];

            const greenCardDisplay = formatValue(greenCardActual);
            const yellowCardDisplay = formatValue(yellowCardActual);
            const orangeCardDisplay = formatValue(orangeCardActual);
            const redCardDisplay = formatValue(redCardActual);
            
            document.getElementById('detailed-stats').innerHTML = `
                <div class="detailed-stat-card-v2 card-green">
                    <div>
                        <div class="stat-number">${greenCardDisplay}</div>
                        <div class="card-label">Green Cards</div>
                    </div>
                    <div class="card-stack-wrapper">
                        <div class="stack-shape stack-dark"></div>
                        <div class="stack-shape stack-medium"></div>
                        <div class="stack-shape stack-light"></div>
                    </div>
                </div>
                <div class="detailed-stat-card-v2 card-yellow">
                    <div>
                        <div class="stat-number">${yellowCardDisplay}</div>
                ...
                <div class="detailed-stat-card-v2 card-orange">
                        <div class="stat-number">${orangeCardDisplay}</div>
                </div>
                <div class="detailed-stat-card-v2 card-red">
                    <div class="stat-number">${redCardDisplay}</div>
                    <div class="card-stack-wrapper">
                        <div class="stack-shape stack-dark"></div>
                        <div class="stack-shape stack-medium"></div>
                        <div class="stack-shape stack-light"></div>
    ...
    function renderLeaderboard() {
            ...
            const leaderboardData = employees.map(emp => {
                // Рахуємо кількість зелених карток
                const greenCardsCount = (emp.greenCards || []).length;
                
                // Отримуємо дату останньої зеленої картки та форматуємо її
                const lastGreenCard = emp.greenCards && emp.greenCards.length > 0 
                    ? emp.greenCards.reduce((latest, gc) => new Date(gc.date) > new Date(latest.date) ? gc : latest)
                    : null;
                ...
                if (lastGreenCard && lastGreenCard.date) {
                    ...
                    if (typeof raw === 'string') {
                        // Якщо це ISO рядок, обрізаємо до дати
            ...
            let filteredData = leaderboardData.filter(emp => {
                ...
                if (selectedPeriod === 'all') {
                    ...
                    // Для періодів (Week, Month) фільтруємо за кількістю зелених карток
            ...
            filteredData.sort((a, b) => b.greenCardsCount - a.greenCardsCount || a.name.localeCompare(b.name));
