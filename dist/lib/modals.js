(function registerModals(global) {
    function showEmployeeModal(data) {
        ...
        const violationsList = (employee.violations && employee.violations.length > 0)
            ? `<div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700"><h4 class="font-semibold mb-2">Violations:</h4><ul class="space-y-2 text-sm text-gray-600">${employee.violations.filter(v => v).map(v => {
                const raw = v.date;
                let displayDate = '';
        ...
        const greenCardsList = (employee.greenCards && employee.greenCards.length > 0)
            ? `<div class="mt-4 pt-4 border-t border-gray-200 dark:border-gray-700"><h4 class="font-semibold mb-2">Green Cards:</h4><ul class="space-y-2 text-sm text-gray-600">${employee.greenCards.filter(gc => gc).map(gc => {
                const raw = gc.date;
                let displayDate = '';
                if (typeof raw === 'string') {
                    displayDate = raw.includes('T') ? raw.split('T')[0] : raw;
