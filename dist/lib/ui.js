(function registerUI(global) {
    ...
    const defaultMonthFormatter = (date) => date.toLocaleDateString('en-US', { month: 'long', year: 'numeric' });
    function formatMonthLabel(date) {
        const formatter = typeof global.formatMonthYear === 'function' ? global.formatMonthYear : defaultMonthFormatter;
        return formatter(date);
    }
    function updateCalendarHeader(elementId, date) {
        if (!date)
            return;
    ...
    function getPrimaryCalendarDate() {
        if (global.currentDate instanceof Date)
            return global.currentDate;
        if (typeof currentDate !== 'undefined')
            return currentDate;
        return undefined;
    }
    function getActiveGreenDate() {
        if (typeof window.currentDateGreen !== 'undefined')
            return window.currentDateGreen;
        if (typeof currentDateGreen !== 'undefined')
            return currentDateGreen;
        return undefined;
    }
    ...
    function getTabIndicatorElements() {
        const container = document.querySelector(TAB_CONTAINER_SELECTOR);
    ...
    function updateTabIndicator(targetButton) {
        ...
        if (!container || !indicator)
            return;
        const activeButton = targetButton || container.querySelector('.tab-btn.active');
        if (!activeButton) {
    ...
    function getPeriodIndicatorElements() {
        const container = document.querySelector(PERIOD_CONTAINER_SELECTOR);
        const indicator = document.getElementById(PERIOD_INDICATOR_ID);
        return { container, indicator };
    }
    function updatePeriodIndicator(targetButton) {
        const { container, indicator } = getPeriodIndicatorElements();
        if (!container || !indicator)
            return;
        const activeButton = targetButton || container.querySelector('.period-btn.active');
        if (!activeButton) {
            indicator.style.opacity = '0';
            return;
        }
    ...
    global.addEventListener('dashboard:periodfilterchange', (event) => {
